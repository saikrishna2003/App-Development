...
function CustomerCrud() {
  ...
  async function editBook(book) {
   ...
   const DeleteBook =(bid)=> {
    console.log(bid)
    try {
        axios.delete(`http://localhost:8080/books/delete/${bid}`,
        {
          headers: {
            'Authorization': `Bearer ${token}` // Include the token here
          }
  ...
  async function update(event) {
    event.preventDefault();
    try {
      await axios.put(`http://localhost:8080/books/put/${bid}`,
        {
          bookname: bookname,
          bookauthor: bookauthor,
          edition: bookedition,
          ...
          imgURl: imgURl,
          videoURl: videoURl,
          bid: bid // Add the bookid to the request body
        },
        {
          headers: {
      ...
      <div className="bigcontainer mt-4">
        <form>
          ...
          <div className="form-group">
            ...
            <input type="text" className="form-control" id="bookimageUrl"
              value={imgURl}
              onChange={(event) => {
          ...
          <div className="form-group">
            <label>Book Video URL</label>
            <input type="text" className="form-control" id="bookvideoUrl"
              value={videoURl}
              onChange={(event) => {
      ...
      <table className="table table-dark" align="center">
        <thead>
          <tr>
            ...
            <th scope="col">Book Image URL</th>
            <th scope="col">Book Video URL</th>
